...
public final class NuHash implements Destroyable {
	...
	private static void substitute_bytes(final long[] buffer) {
		for (int pos = 0; pos < buffer.length; ++pos) {
			final long value = buffer[pos];
			long result = 0L;
			for (int offset = 0; offset < Long.SIZE; offset += Byte.SIZE) {
				result |= (NUHASH_SBX[(int)((value >>> offset) & 0xFF)] & 0xFFL) << offset;
			}
			buffer[pos] = result;
		}
	}
